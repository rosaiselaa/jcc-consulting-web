...
const corporateTools = [
  { icon: Database, accent: "blue", image: sapToolImage, slug: "sap-pmo", title: "SAP ERP", text: "Integración centralizada de los flujos de operaciones, logística y finanzas en una sola fuente de verdad corporativa.", detail: "Acompañamos la implementación desde el lado de la operación para que SAP responda a la realidad de la mina. Ordenamos procesos, costos, CAPEX/OPEX y responsabilidades antes de configurar, y acompañamos la adopción de los usuarios clave.", tag: "SAP PMO MINERO" },
  { icon: BarChart3, accent: "gold", image: powerBiToolImage, slug: "power-bi", title: "Power BI & Business Intelligence", text: "Dashboards ejecutivos con KPIs analíticos en tiempo real para la toma de decisiones gerenciales.", detail: "Conectamos indicadores operativos y financieros en una lectura ejecutiva que permite pasar de la desviación a la acción. Diseñamos dashboards con contexto, responsables y seguimiento para que los datos ayuden a decidir, no solo a reportar.", tag: "BUSINESS INTELLIGENCE" },
...
export default function Technology() {
  ...
  const [activeToolSolution, setActiveToolSolution] = useState<string | null>(null);
  ...
  const selectedToolSolution = corporateTools.find(tool => tool.title === activeToolSolution);
  const selectedSolution = selectedTransformation ?? selectedToolSolution;
  const solutionTitle = selectedSolution && ("label" in selectedSolution ? selectedSolution.label : selectedSolution.title);

  useEffect(() => {
    if (!activeTransformation && !activeToolSolution) return;
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape") {
        setActiveTransformation(null);
        setActiveToolSolution(null);
      }
    };
